...
def convert_audio_channels(wav, channels=2):
    ...
    *shape, src_channels, length = wav.shape
    #print(*shape, src_channels, length)
...
def convert_audio(wav, from_samplerate, to_samplerate, channels):
    ...
    resampler = T.Resample(orig_freq=from_samplerate, new_freq=to_samplerate)
    waveform_resampled = resampler(wav)
...
class LowPassFilters(torch.nn.Module):
    ...
    def __init__(self, cutoffs: list, width: int = None):
        ...
        self.register_buffer("filters", torch.stack(filters).unsqueeze(1).to(dtype=torch.float32))
