...
class Solver(object):
    ...
    def _reset(self):
        ...
        if load_from:
            ...
            #package = torch.load(load_from, 'cpu's)
            package = torch.load(load_from, map_location='cpu')

            if load_best:
                self.model.load_state_dict(package['best_state'])
            else:
                self.model.load_state_dict(package['model']['state'], strict=False)
    ...
    def train(self):
        ...
        for epoch in range(len(self.history), self.epochs):
            ...
            train_loss = self._run_one_epoch(epoch, bone_conduction=True) #set to False if not using bone conduction
            logger.info(
            ...
            if self.cv_loader:
                ...
                self.model.eval()
                with torch.no_grad():
                    valid_loss = self._run_one_epoch(epoch, cross_valid=True, bone_conduction=True) #change to  bone_conduction=False if no bc
                logger.info(
    ...
    def _run_one_epoch(self, epoch, cross_valid=False, bone_conduction=False):
        total_loss = 0
        ...
        for i, data in enumerate(logprog):
            if bone_conduction:
                noisy, clean, bone = [x.to(self.device) for x in data]
            ...
            if not cross_valid:
                ...
                noisy = noise + clean
            if bone_conduction:
                estimate = self.dmodel(noisy, bone)
            else:
