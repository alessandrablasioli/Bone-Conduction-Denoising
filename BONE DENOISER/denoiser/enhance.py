# Copyright (c) Facebook, Inc. and its affiliates.t
...
group = parser.add_mutually_exclusive_group()
group.add_argument("--bone_dir", type=str, default=None,
                   help="directory including noisy wav files")
group.add_argument("--bone_json", type=str, default=None,
                   help="json file including noisy wav files")

def get_estimate(model, noisy, bone, args):
#def get_estimate(model, noisy,  args):
    ...
    if args.streaming:
        ...
        with torch.no_grad():
            estimate = torch.cat([
                streamer.feed(noisy[0], bone[0]),
                #streamer.feed(noisy[0]),
                streamer.flush()], dim=1)[None]
    else:
        with torch.no_grad():
            estimate = model(noisy, bone)
...
def get_dataset(args, sample_rate): 
#def get_dataset(args, sample_rate): 
    ...
    '''
    if paths.bone_json:
        with open(paths.bone_json) as f:
            files_b = json.load(f)
    elif paths.bone_dir:
        files_b = find_audio_files(paths.bone_dir)
        ...
        return None
    '''  
    #return Audioset(files, with_path=True,sample_rate=sample_rate, convert=True) #,channels=channels,channels=channels, 
    return Audioset(files, with_path=True,sample_rate=sample_rate,  convert=True), Audioset(files, with_path=True, sample_rate=sample_rate, convert=True) #files_b
...
'''
def _estimate_and_save(model, noisy_signals, bone_signals, filenames, files_b, out_dir, args):
    estimate = get_estimate(model,noisy_signals,bone_signals, args)
    save_wavs(estimate, noisy_signals, filenames, out_dir, sr=model.sample_rate)
...
def enhance(args, model=None, local_out_dir=None):
    ...
    #noisy and bone dataset
    #dset = get_dataset(args, model.sample_rate) 
    dset, dset_b  = get_dataset(args, model.sample_rate) #, model.chin
    if dset is None:
        return
    ...
    #loader for bone conduction file
    loader_b = distrib.loader(dset_b, batch_size=1)
    ...
    with ProcessPoolExecutor(args.num_workers) as pool:
        #iterator = LogProgress(logger, loader, total=total, name="Generate enhanced files")
        iterator = LogProgress(logger, zip(loader, loader_b), total=total, name="Generate enhanced files") 
        pendings = []
        for data in iterator:
            # Get batch data
            #noisy_signals, filenames = data
            (noisy_signals, filenames) , (bone_signals, files_b) = data  
            noisy_signals = noisy_signals.to(args.device)
            bone_signals = bone_signals.to(args.device)
            if args.device == 'cpu' and args.num_workers > 1:
                pendings.append(
                    #pool.submit(_estimate_and_save,model, noisy_signals,filenames,  out_dir, args))
                    pool.submit(_estimate_and_save, model, noisy_signals,bone_signals,filenames, files_b,  out_dir, args))
            else:
                # Forward
                estimate = get_estimate(model, noisy_signals, bone_signals, args)
                #estimate = get_estimate(model, noisy_signals,  args)
                save_wavs(estimate, noisy_signals, filenames, out_dir, sr=16000)
