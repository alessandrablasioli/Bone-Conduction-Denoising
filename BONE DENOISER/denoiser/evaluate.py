...
def evaluate(args, model=None, data_loader=None):
    ...
    if data_loader is None:
        dataset = NoisyCleanSet(args.data_dir, dataset='vibravox', 
                                matching=args.matching, sample_rate=model.sample_rate, bone_conduction=True)
    ...
    with ProcessPoolExecutor(args.num_workers) as pool:
        with torch.no_grad():
            ...
            for i, data in enumerate(iterator):
                # Get batch data
                noisy, clean,bone = [x.to(args.device) for x in data]  
                ...
                if args.device == 'cpu':
                    pendings.append(
                        ...
                        pool.submit(_estimate_and_run_metrics, clean, model, noisy, bone, args))
                else:
                    estimate = get_estimate(model, noisy, bone, args)
...
'''
def _estimate_and_run_metrics(clean, model, noisy, bone, args):
    estimate = get_estimate(model, noisy, bone, args)
    return _run_metrics(clean, estimate, args, sr=model.sample_rate) 
